"""Tests for `research/enrich.py::enrich_context` -- DESIGN section 5.2 steps
10-12 (Session 9, Task 8): cost application, context tagging, split
assignment.
...
`earnings_in_window` is checked against `ExitParams.max_hold_days` -- the
FIXED analysis window, not the trade's realized `holding_days` (fixed in
response to code review Finding 1; see `_earnings_in_window`'s docstring
in `research/enrich.py` for the full reasoning: `touched_5pct` /
`day_touched_5pct` on the same row are computed over the full fixed
window regardless of when the exit fired, so this flag must describe the
same window, not a shorter realized one).
"""
...
class TestCostsAlwaysSubtract:
    def test_losing_long_gets_worse_with_costs(self):
        ...
        ind_row = _ind_row()
        plain = enrich_context(event, ind_row, None, _SP, _SPLITS, _ZERO_COST, _EP)
        costed = enrich_context(event, ind_row, None, _SP, _SPLITS, _REAL_COST, _EP)
    ...
    def test_short_pays_strictly_more_than_an_identical_long_via_borrow(self):
        ...
        # must be lower than the long's, since slippage and commission are
        # side-symmetric (DESIGN section 3.9) and only the short pays borrow.
        ...
        ind_row = _ind_row()
        long_out = enrich_context(long_event, ind_row, None, _SP, _SPLITS, _REAL_COST, _EP)
        short_out = enrich_context(short_event, ind_row, None, _SP, _SPLITS, _REAL_COST, _EP)
    ...
    def test_net_ret_is_nan_when_position_never_resolved(self):
        ...
        out = enrich_context(event, _ind_row(), None, _SP, _SPLITS, _REAL_COST, _EP)
...
class TestDdBucketMatchesCompute:
    ...
    def test_matches_compute_dd_bucket_for_representative_values(self, dd_52w):
        ...
        out = enrich_context(event, ind_row, None, _SP, _SPLITS, _ZERO_COST, _EP)
    ...
    def test_dd_bucket_labels_are_derived_not_restated(self):
        ...
        out = enrich_context(event, ind_row, None, custom_sp, _SPLITS, _ZERO_COST, _EP)
...
class TestSplitKeyMatchesTask2:
    ...
    def test_split_key_matches_split_key_for(self, signal_date):
        ...
        out = enrich_context(event, _ind_row(), None, _SP, _SPLITS, _ZERO_COST, _EP)
    ...
    def test_split_key_raises_below_event_start_same_as_split_key_for(self):
        ...
        with pytest.raises(ValueError, match="2009-06-15"):
            enrich_context(event, _ind_row(), None, _SP, sp_splits, _ZERO_COST, _EP)


# ---------------------------------------------------------------------------
# 4. earnings_in_window -- null (not False) when days_to_earnings is null,
#    and checked against the FIXED window (ExitParams.max_hold_days), never
#    the trade's realized holding_days (Finding 1 fix).
# ---------------------------------------------------------------------------
...
class TestEarningsInWindow:
    ...
    def test_true_when_earnings_falls_inside_the_fixed_window(self):
        event = _event(holding_days=5)
        ind_row = _ind_row(days_to_earnings=3)
        out = enrich_context(event, ind_row, None, _SP, _SPLITS, _ZERO_COST, _EP)
        assert out["earnings_in_window"] is True

    def test_true_at_the_max_hold_days_boundary(self):
        ep = ExitParams(max_hold_days=5)
        event = _event(holding_days=5)
        ind_row = _ind_row(days_to_earnings=5)
        out = enrich_context(event, ind_row, None, _SP, _SPLITS, _ZERO_COST, ep)
        assert out["earnings_in_window"] is True

    def test_false_when_earnings_falls_outside_the_fixed_window(self):
        ep = ExitParams(max_hold_days=5)
        event = _event(holding_days=5)
    ...
    def test_true_when_position_never_resolved_but_earnings_is_within_the_fixed_window(self):
        # holding_days is None (unresolved position) no longer nulls this
        # flag -- the fixed window doesn't depend on how the trade resolved.
        ep = ExitParams(max_hold_days=5)
    ...
    def test_earnings_window_moves_with_ep_max_hold_days_not_a_literal(self):
        # Proof this isn't hardcoded to 5 (invariant 9): shrinking the
        # configured window pushes a previously-in-window report out.
        ep = ExitParams(max_hold_days=3)
        event = _event(holding_days=5)
        ind_row = _ind_row(days_to_earnings=4)
        out = enrich_context(event, ind_row, None, _SP, _SPLITS, _ZERO_COST, ep)
        assert out["earnings_in_window"] is False

    def test_stopped_out_early_with_earnings_still_inside_the_fixed_window_is_true(self):
        # Finding 1's exact regression case: the position stopped out on
        # day 2 (holding_days=2), earnings lands on day 4, and the fixed
        # analysis window is the default max_hold_days=5. The reachability
        # columns (touched_5pct / day_touched_5pct) on this same row were
        # computed by path_metrics over the FULL [t+1, t+5] window
        # regardless of the day-2 exit (DESIGN section 5.6), so
        # earnings_in_window must describe that same fixed window -- True
        # here -- not the shorter realized holding_days=2, which would
        # incorrectly say False (2 < 4) even though days 3-5 of this row's
        # reachability data are earnings-driven.
        event = _event(holding_days=2)
        ind_row = _ind_row(days_to_earnings=4)
        out = enrich_context(event, ind_row, None, _SP, _SPLITS, _ZERO_COST, _EP)
        assert out["earnings_in_window"] is True
...
class TestEra:
    ...
    def test_era_boundaries_move_with_stats_params(self):
        ...
        out = enrich_context(event, _ind_row(), None, custom_sp, splits, _ZERO_COST, _EP)
...
class TestMarketRow:
    ...
    def test_read_from_market_row_when_present(self):
        ...
        out = enrich_context(event, _ind_row(), market_row, _SP, _SPLITS, _ZERO_COST, _EP)
