...
| `cscan nightly`, cold | **~30 min** measured 2026-09-04 (29m54s) and 2026-09-07 (31m42s) from `runs`; the older 35-40 min figure was never measured. **Add ~11 min** now that `predict` is in the chain, and **~15 min more** for the `next_open` resolution step, measured end to end on 2026-09-15 (12m17s backtest at 40 tickers, then 2m30s of harness — the harness is easy to forget and it is a sixth of the step). Whole chain that night: **42m27s**, 21:47:15 to 22:29:42 PT. A bad night is longer: 2026-09-08 took 1h53m when `path_capture` hit the cosmetic scope. **On 2026-09-15 that step was uncapped, selected 675 tickers, and was killed by `RuntimeMaxSec=4h` twice without reaching `sync`** — a step that picks its own work needs its own cap. → `OPERATIONS.md` |
