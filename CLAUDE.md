...
Read `docs/DECISIONS.md`. It holds 201 ADRs. They are decisions, not suggestions.
...
after that ADR was amended twice and the clause withdrawn; work started
against it before anyone read the ADR. Check the ADR, not the backlog
summary of it.
...
- **`cscan indicators` writes per chunk, and this line said the opposite until 2026-09-24.** It collected across every ticker and upserted once until 2026-08-26, when it moved to `INDICATOR_CHUNK_SIZE` tickers per write -- because that shape decayed from ~900 rows/s to **~46** as memory filled, and because **two working runs were killed on 2026-08-25** by someone reading a mid-run count of zero as a hang. This file then kept telling the next reader to expect exactly that. Rows appear per chunk, so a mid-run count is real progress. Pass `--workers 8`; it defaults to 1.
- **`cscan events` takes `--workers` as of 2026-09-22, and the default of 1 is deliberate.** Parallelism does nothing at five tickers (13.3 s against 15.9 s) and 16.6x at twenty, because spawn startup and each worker's own reads dominate the small case -- which is exactly the nightly's shape. Use it for a rebuild, not for the schedule. The same run measured 337.9 s before this work, 39.4 s serial and 20.3 s at `--workers 8`, with byte-identical output. -> `TIMINGS.md`
...
7. **No broker client, no order placement, no brokerage credentials.** The absence is the safety property, not a disabled flag. **This collides with a planned feature and the collision is deliberate, not an oversight:** ADR 048 sizes `positions.source` for `broker_synced`, and the owner intends a Schwab read-only sync (2026-09-25). Read-only still means OAuth tokens, which this invariant forbids as written. **Amending it needs its own ADR** -- not a quiet addition while building the client. -> `BACKLOG.md`
