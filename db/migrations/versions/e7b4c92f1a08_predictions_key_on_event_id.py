"""predictions key on the event, not on ticker and date

**A ticker can fire twice on one day, and the two can be opposite sides.**
Fourteen ticker-days in the two months to 2026-09-05 carry two `next_open`
events each, and the examples are not edge cases -- BNS on 2026-08-25 has a
`bb_upper_touch` short and a `stoch_oversold` long; DDOG on 2026-08-06 has a
`bb_lower_touch` long and a `stoch_overbought` short.

That breaks `(ticker, as_of)` as a key for `predictions`, and migration
`c3f8a1e07b26` made it a unique index on exactly that pair. The immediate
symptom was loud and harmless: `cscan predict` died with
`ON CONFLICT DO UPDATE command cannot affect row a second time`, because
one batch proposed two rows for one key.

**The quiet symptom is the one that matters.** `p_touch` is the probability
of a favourable excursion *in the direction the signal assigned*. Had the
writer deduplicated and kept one row per ticker-day, the screener -- which
joins on ticker and date and renders a `side` per row -- could show a long
row the probability computed for the short. A number that is confidently
about the opposite trade is worse than no number, and nothing on screen
would distinguish the two.

So the key is `event_id`, which `predictions` has carried since migration
005 and which is exact: one prediction per event, no collapsing, no
preference rule to get wrong. Both screener views join on it instead.

`(ticker, as_of)` survives as a **non-unique** index, because
`handlers.predict` still looks up that way. When two sides exist that
lookup is genuinely ambiguous and resolves by `as_of DESC, id DESC`, which
is deterministic rather than correct -- the handler cannot know which side
the caller meant. → `BACKLOG.md`

Revision ID: e7b4c92f1a08
Revises: d5a02b18c937
...
revision = "e7b4c92f1a08"
down_revision = "d5a02b18c937"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    # Drop the views first: a view depending on `predictions` does not block
    # an index change, but rewriting them after the key change keeps the two
    # halves of this migration in one transaction and one reading order.
    ...
    """)
    op.execute("""
        CREATE UNIQUE INDEX IF NOT EXISTS predictions_event_id
            ON predictions (event_id)
    """)
    # Still the handler's lookup path, no longer a uniqueness claim.
    op.execute("""
        CREATE INDEX IF NOT EXISTS predictions_ticker_as_of
            ON predictions (ticker, as_of DESC)
    """)
...
def downgrade() -> None:
    ...
    op.execute("DROP INDEX IF EXISTS predictions_event_id")
    op.execute("DROP INDEX IF EXISTS predictions_ticker_as_of")
    op.execute("""
        DELETE FROM predictions a USING predictions b
