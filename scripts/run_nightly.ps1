...
# **One log per day under `reports/nightly/`.** The directory is created if
# missing so a fresh clone does not lose the run to a path that is not
# there yet -- the same trap `wait_and_poll.ps1` hit with `reports/poller/`.
$logDir = "reports\nightly"
...
# **Read the expected hash, never hardcode it.** This guard held
# `a38d3ca6b58295e8` as a literal, and when the exit sweep moved the config
# on 2026-08-29 it would have refused every nightly from then on -- while
# naming the *correct* new hash as the problem. `serving_config` is the
# authority (ADR 115): it is what the site reads and what
# `cscan db sync-config` writes, so a guard reading it cannot disagree with
# what is actually being served.
#
# Falls back to the resolved hash when serving is unreachable, making the
# check a no-op rather than a false refusal -- a sleeping Pi must not stop
# the workstation's nightly.
$expected = & .\.venv\Scripts\python.exe -c "from capitalscan.jobs import sync as s; from sqlalchemy import text; c=s.serving_engine().connect(); print(c.execute(text('SELECT config_hash FROM serving_config')).scalar_one())" 2>$null
...
if ($hash -ne $expected) {
    ...
    "REFUSING: config resolves to $hash, not the serving generation $expected, and no config.toml explains it. An ablation arm is probably set in core/config.py." | Write-Log
