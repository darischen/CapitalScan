"""screener views expose the per-field calibration

ADR 175. The views already carry `p_adverse_3`, and until now there was no
way to display it: invariant 8 requires a probability to arrive with
`n_eff` and an interval, and the only interval on the row described
`p_touch_3`. Rendering the adverse probability beside the touch column's
interval would have satisfied the invariant on paper while describing a
different quantity, computed over a different reliability table -- correct
looking, and wrong.

`predictions.calibration_json` (migration `a9d3e05f7b21`) holds one block
per published field, so this exposes it and the UI reads the band it needs.

**`CREATE OR REPLACE VIEW` matches columns positionally**, which is why the
new column goes at the very end of the select list rather than beside
`p_adverse_3` where it reads better. An earlier migration in this series
learned that with `cannot change name of view column`.

Revision ID: b4f8c17d29e6
Revises: a9d3e05f7b21
...
revision = "b4f8c17d29e6"
down_revision = "a9d3e05f7b21"
branch_labels = None
depends_on = None
